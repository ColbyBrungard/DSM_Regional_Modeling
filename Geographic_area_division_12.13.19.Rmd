...
```{r}
ucrb <- readOGR("./Boundaries", "CO_River_watershed_Meade_NAD83_EcoL3_dissolve")
...
```{r}
ucrb.9 <- stratify(ucrb.poly, nStrata = 9, nTry = 1, cellSize = 5000) # cell size was set to 1000 for analysis, but here cell size is set to 5000 to ease plotting. 
plot(ucrb.9)
...
```{r}
ucrb.8 <- stratify(ucrb.poly, nStrata = 8, nTry = 1, cellSize = 5000)
plot(ucrb.8)
# Convert geographical strata to raster so that I can overlay points
# First convert to SpatialPixelsDataFrame
...
```{r}
ucrb.4 <- stratify(ucrb.poly, nStrata = 4, nTry = 1, cellSize = 5000)
plot(ucrb.4)
# Convert geographical strata to raster so that I can overlay points
# First convert to SpatialPixelsDataFrame
...
# write to shapefile. I then dissolved these in QGIS to only get the areas (which I needed to do for calculating area). 
# st_write(s9_sf, "Z:/UCRB/Boundaries/geographic_9.shp")
# st_write(s8_sf, "Z:/UCRB/Boundaries/geographic_8.shp")
# st_write(s4_sf, "Z:/UCRB/Boundaries/geographic_4.shp")
...
gar
ggsave("./geographicAreas_2.png", plot = gar)
```
